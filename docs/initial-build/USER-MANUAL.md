...
**Clearing everything at once.** **Reset search & filters** sits on the **Filters** bar itself, at the right-hand end, so you can clear the view without opening the panel first — useful when you come back from a brother's page and want to start again. It empties the Name Search box, clears every filter, returns the sort to name order, and turns off both **Include deceased** and **Starred only** — everything that narrows who appears in the list. It leaves your choice of columns alone, and it leaves any rows you have selected selected (those have their own **Clear selection**). The button is greyed out when there is nothing to clear.
...
**Emailing a group of brothers.** The checkbox at the left of each row selects that brother, and the box in the heading row selects everyone currently listed. With brothers selected, **Copy Emails** puts their email addresses on your clipboard, ready to paste into the To: line of an email — so you can write to the classes of 2010–2014, or everyone in Vermont, without copying addresses out one at a time. Your selection is kept as you search and filter, so you can gather brothers from several different views before you copy; the count on the button tells you how many you have, and **Clear selection** empties it.

Some brothers are always left out, and the message afterwards tells you how many and why: brothers with no email address on file, brothers who have asked to keep their address private, and brothers who have passed. **You can copy up to 50 brothers at a time.** If you need a longer list — for a class-wide mailing, say — ask a manager or administrator, who can copy any number.
...
If you help maintain the membership, you may have a **manager** role, which the badge by your name at the top of the page will show. Managers can see everything a brother sees, plus a few additional fields that help with membership upkeep, and managers can **export** a list from the Directory for offline work. The action bar above the grid has an **Export CSV** button: it produces a spreadsheet of the rows you have **selected** (using the checkbox column at the left edge, with a select-all in its header), or — if you've selected nothing — of the whole list as you've currently filtered it.

Export offers **two spreadsheets**, and the first is usually the one you want. **Export displayed columns** gives you exactly the columns you have on screen, in the order you have them, reading as they read here — so if you have narrowed the Directory to a class year and put Email and Telephone on screen, that is the file you get, with nothing to tidy up afterwards. **Export all data** gives you every field your role can see: the fuller file to keep as a record, or to load into another system. Either way the export contains only the information your role is allowed to see, and never includes photographs. Managers see a brother's privacy and consent settings and the dates on his record, but — like everyone else — a manager does not see a contact detail a brother has chosen to keep private; only an administrator can see through those settings. Managers can correct a brother's information; note that correcting another brother's profile removes its "verified" mark. Managers can also **confirm** a record is current on a brother's behalf, from the record-status area of his profile, which marks it verified again.
