...
User-facing paths and data endpoints are different namespaces served by different origins behind one load-balancer URL map:

- `/api/*` → the Node/TypeScript backend on Cloud Run. **Every endpoint in this document lives under `/api`.**
- `/img/*` → the image bucket via Cloud CDN (headshots/thumbnails; see §6).
...
- **Auth:** any authenticated user.
- **Behavior:** invalidates the server-side session and clears the session and CDN signed cookies (and may emit `Clear-Site-Data`).
- **Response 204.**
...
> Profile *edits* also affect verification automatically (the `PATCH` side-effect above, decision D28). This explicit action covers the cases edits do not: the owner confirming with nothing to change, and a manager/admin re-verifying after an edit (with the cautioned copy).
...
- **Behavior — raising (`true`):** snapshots the brother's consent/verification state (system-internal, DATABASE-SCHEMA §8), sets `debrothered.isDebrothered = true` (with `debrotheredAt`), and **deletes the Ghost member via the Ghost-first delete path** (decision D98 — abort-clean on Ghost failure, so Book is untouched if Ghost fails). From then on the record is projected away for brothers (§1.3/§3) and sign-in for the resolved profile is denied (§2). **Reversing (`false`):** restores the snapshot and **re-creates the Ghost member** via the Ghost-first create path (decision D96), then clears the flag. The reconciliation audit treats a de-brothered profile as expected to have no Ghost member (no `missingGhostMember` drift, decision D99).
...
- **Behavior:** changes the target's `role`, subject to a **server-enforced last-admin invariant** — the demotion of the **only remaining admin** is rejected (a direct API call must not be able to lock the org out of backup/restore, add/delete, role changes, and Ghost sync). Every role change is **audit-logged with the before and after role** (decision D106); this entry also feeds the D101 forensic privileged-roster log and the D105 detect-and-reverse posture.
- **Response 200:** `{ "id": 5305, "role": "manager" }`
- **Errors:** `404` if no such user; `422` on an invalid role value; `409 { "error": "last_admin" }` if the change would demote the only remaining admin.

## 6. Headshots & thumbnails

**Reads have no backend endpoint.** Both images are immutable, versioned, CDN-served GCS objects (DATABASE-SCHEMA §7) at `/img/headshots/{id}/{version}.webp` and `/img/thumbnails/{id}/{version}.webp`. The SPA builds these URLs itself from `id` and `headshotVersion`; the browser attaches the Cloud CDN **signed cookie** (set at session establishment, §2) and the edge serves or refuses the object. The signed cookie is a coarse grant — any authenticated brother may view any brother's photos (decision D23) — and its lifetime matches the session (4 hours), reissued whenever the session is re-established.

The backend touches images only on the **write** path:
