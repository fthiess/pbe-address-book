...
- **Caching:** served **`no-store`** — like `/api/profiles`, this response now carries the caller's own contact values, so D95's no-disk-PII rationale applies. As there, the *delivered* header is enforced by `firebase.json`'s `/api/**` rule (**D146**); this one is the direct-to-Cloud-Run floor.
...
- **Caching:** served **`Cache-Control: no-store`** — the payload is real PII (every brother's name, plus shared emails/phones) and must never persist to a shared machine's disk; it is held only in memory while the tab is open. This **supersedes D76's read-side *browser-cache* conditional GET** (decision D95); the write-side `If-Match`/`updateTime` OCC of §1.4 is unaffected. **The *delivered* header is enforced by the `/api/**` rule in `firebase.json`, not by this line** — Firebase Hosting overrides a backend `Cache-Control` on 2xx rewrite responses, so until **D146** this endpoint shipped `no-cache, must-revalidate` and the whole roster was cacheable to disk. The app header remains defense-in-depth for direct-to-Cloud-Run access.
- **Conditional read (Phase 7.5b, decisions D159/N148):** the `200` carries an **`ETag`** — a quoted, role-qualified dataset version token (`"{hash16}.{role}"`, a truncated SHA-256 content hash of the served JSON; ⚠ *not* the per-record `updateTime` token of §1.4, a different `ETag` at a different granularity). A request may send it back as **`If-None-Match`**; on a match the server answers **`304 Not Modified`** with an empty body, the same `ETag`, and **`Cache-Control: no-store` still set** — the revalidation is **application-level over the SPA's heap-only store**, not browser caching, which is what keeps this compatible with D95 (N63). The matcher tolerates quotes, a `W/` prefix, and comma-lists, and deliberately does **not** honor `*`. The token is qualified by **effective role** (N31), so one role's token never `304`s another role's projection; any write to any profile changes the dataset hash, so a stale token simply receives the full `200` with the current token.
- **Response 200:** `{ "profiles": [ { /* projected Profile */ }, … ], "majors": [ { /* Major */ }, … ] }`, with the dataset `ETag` header above.
...
- **Response 304:** empty body; `ETag` + `no-store` as above. Returned only when `If-None-Match` matches the caller's current role-qualified token.
...
- **Auth:** any authenticated user.
- **Response 200:** a single projected `Profile`, with an `ETag` header carrying the concurrency token (§1.4). For the **brother** role the body may additionally carry **`hiddenLittleBrothers`** (decision D168): a count of the brothers who name this record as their Big Brother but are themselves projected away from this caller. It is **derived per request, not a `Profile` field** — it describes *other* records, is absent for managers and admins (who resolve the whole edge from their own roster) and absent when the count is zero, and is never accepted on a write. ⚠ **Every endpoint that returns a projected record carries it** — this read, the `PATCH` reply, and the create `201` — not just this one: the `PATCH` reply is the record the client *keeps* (decision N33 retired the post-save `GET` precisely because it substitutes for one) and the client replaces its held record wholesale rather than merging (OFC-137), so a count supplied only here silently vanished on the first save. Without it the SPA cannot know a withheld Little Brother exists at all: the Little-Brother edge is derived client-side by filtering the roster on `bigBrotherId`, and a hidden record is absent from that roster entirely (the Big-Brother direction needs no such help, because `bigBrotherId` is a `public` field on the record being read).
- **Errors:** `404` if no such brother. For a **brother**, a **de-brothered** (decision D115) *or* **Unlisted** (decision D124) record is projected away and returns **`403 private`** — distinct from the `404` above, and deliberately identical between the two hide reasons so the caller never learns which applies (decision D168, resolving the "404/403" D124 left open); managers and admins receive it normally.
- **Caching:** served **`Cache-Control: no-store` on *every* branch** — the `200` (real PII, D95) **and** the error responses (`403`/`404`/`401`/`400`). This read's visibility is **per-role**: the same URL is a `403` for a brother and a `200` for an admin, so a cacheable `403` could be replayed by a shared cache to a higher-role caller and the record would "disappear" for them (decision N75). Firebase Hosting injects a default `Cache-Control: max-age=600` on `/api/**` rewrite responses that set none, so the `no-store` must be explicit on the error paths, not just the success path (this manifested as OFC-192). ⚠ **That is only half of Hosting's behaviour, and the half-picture is what let D95 go unenforced for months:** on **2xx** responses Hosting does not merely fill a gap, it **overrides** an explicit backend `Cache-Control` — so before **D146** this endpoint's `200` was *delivered* `no-cache, must-revalidate` despite the app setting `no-store`. Errors (4xx/5xx) pass through untouched, which is exactly why the OFC-192 investigation, working from a `404`, concluded the opposite. The delivered value is now pinned by the `/api/**` rule in `firebase.json` (D146); the header set here remains the floor for direct-to-Cloud-Run access.
...
|---|---|---|---|
| `/api/admin/backup` | GET | MVP | Download a full database backup (text + images); the automated twice-daily backup runs separately, over `POST /api/internal/backup` (§8.2, decision D63). The admin is **custodian** of the downloaded archive (decision D101; USER-MANUAL). |
...
| `/api/admin/restore` | — | **offline (no API endpoint)** | Restore is an **offline maintenance event** (decisions D100/D101): Book goes hard-down, the three collections are replaced from a **structurally-validated** backup (cycle/ID-uniqueness/email-uniqueness/reference-integrity), and the single instance cold-hydrates on restart. Operator tooling, not an online endpoint. *Built in Phase 7b-3 (decision D150) as `npm run restore --workspace apps/api`; procedure in `infra/README.md`.* |

**Backup envelope (`GET /api/admin/backup` response).** The download is **JSON-only** — the collections snapshot — served as a dated download attachment (`Content-Disposition: attachment; filename="book-backup-YYYY-MM-DD.json"`, `no-store`) and audited as `backup.download` (decisions D63/N58). The body is `{ "version": 2, "generatedAt": "<ISO>", "collections": { "profiles": [{ "id": "<docId>", "data": { … } }], "users": [ … ], "config": [ … ] }, "images": [ … ] }` — each collection is an array of `{ id, data }` documents so a restore is a faithful, key-preserving replay, and `images` is the image-version manifest described in §8.2. *(Envelope **version 2** as of Phase 7b-3, decision D150: this download emitted `version: 1` — `collections` only — while the automated snapshot emitted 2, and D147 parked the unification for the session that would first read both. It emits the same shape as the automated job now, so everything downstream reads one thing; the offline restore still **accepts** version 1, so archives an admin downloaded before that change remain restorable.)* The `bugReports` collection (decision D121) is **not** in the backup: it is transient triage data an admin clears (like `sessions`/`authNonces`), not part of the durable directory a restore reconstructs. The **image-object bundle** (the zipped headshots/thumbnails, decision D63) remains outstanding. The **automated backup** shipped in Phase 7b-2 as its own service-authenticated endpoint — `POST /api/internal/backup`, §8.2 — not as a mode of this one.
...
### 8.2 The automated backup

Cloud Scheduler triggers the whole-database backup over this endpoint, twice daily (decision D149) (decision D63; DR posture D102; ENGINEERING-DESIGN §6.3). It is a **trigger, not a data surface** — it returns counts, never records.
...
- **Snapshot object (version 2)** — written to `gs://<BACKUP_BUCKET>/backups/<ISO-8601 with `:`/`.` as `-`>.json`, plain JSON:
  ...
  ```
  `collections` is exactly the §7 download's shape. `images` is the **image-version manifest** (decision D63): images are never copied into a backup — GCS object versioning preserves them (decision D8) — so the manifest is how a restore pins the right object versions. It is derived purely from the profiles snapshot, since the object key embeds `headshotVersion`; verifying that those objects still exist is the integrity job's business (decision D102), not the backup's. The manual §7 download emitted `version: 1` (`collections` only) until Phase 7b-3 **unified the two envelopes** (decision D150): both producers now emit this shape. The offline restore still reads version 1, so pre-existing archives stay restorable, but nothing produces one.
