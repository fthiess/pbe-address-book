...
- **Caching:** served **`Cache-Control: no-store`** — the payload is real PII (every brother's name, plus shared emails/phones) and must never persist to a shared machine's disk; it is held only in memory while the tab is open and re-fetched on the next fresh load. This **supersedes D76's read-side conditional-GET/`304` caching** (decision D95); the write-side `If-Match`/`updateTime` OCC of §1.4 is unaffected. **The *delivered* header is enforced by the `/api/**` rule in `firebase.json`, not by this line** — Firebase Hosting overrides a backend `Cache-Control` on 2xx rewrite responses, so until **D146** this endpoint shipped `no-cache, must-revalidate` and the whole roster was cacheable to disk. The app header remains defense-in-depth for direct-to-Cloud-Run access.
