...
### `POST /api/profiles`
Create a brother in one atomic write. **Add Brother is a two-step create (decision N71, OFC-201):** a small **essentials form** (`/brother/new`) collects only the identity fields a new record and its Ghost member need — the Constitution `id`, first/last name, class year, and email — POSTs them here, and then hands the admin to the **regular `/brother/:id/edit` page** to fill in everything else. (This replaces the earlier "complete Profile body, no empty-then-fill step" design; the created record is always *valid* because the always-required fields are exactly the essentials collected, so there is no junk-skeleton record and no second parallel create form to maintain.)

- **Auth:** **admin only.**
- **Request:** a `Profile` body carrying at least the caller-supplied Constitution `id` (never auto-assigned; it is the brother's physical Constitution signature number) and the always-required fields (`firstName`, `lastName`, `classYear`); the essentials form also sends `email`. A **partial** body is accepted — the server fills the schema privacy defaults and the status flags, so unsent fields take their defaults. Client-sent **protected / server-managed** fields (`deceased`, `debrothered`, `hasHeadshot`, `headshotVersion`, `lastVerifiedDate`/`verifiedBy`, `lastModified`, `newsletterConsentChangedAt`, `ghostMemberId`) are **ignored**, not honored, and set by the backend.
- **Ghost-first-gated create (decisions D96/N65):** the server creates the Ghost member **first** (capturing `ghostMemberId`, `send_email=false` to suppress the signup email) and folds its id into the write; a Ghost failure aborts clean with `502 { "error": "ghost_create_failed" }` and nothing is written. A brother with **no email** is created **Book-only** (no Ghost member, no `ghostMemberId`) — the reconciliation audit reports it as `missingGhostMember` — mirroring how the PATCH path skips the push for a record with no `ghostMemberId`.
- **Response 201:** the created, projected `Profile`, with an `ETag` header.
- **Errors:** `409` if `id` already exists (the atomic Firestore `create()` is the authoritative guard, with a fast in-memory pre-check); `422` on validation failure (including a missing/invalid `id`); `403` for a non-admin; `502 { "error": "ghost_create_failed" }` if the Ghost-first create fails.
...
- **Ghost push (decisions D96/N65) — Ghost-first-gated:** when the PATCH changes any Ghost-pushed field — `email`, a Canonical-Name input (`firstName`, `lastName`, `classYear`), or `allowNewsletterEmail` — the server pushes the **diff** to the Ghost Admin API (addressed by `ghostMemberId`) **before** committing: Ghost must accept the change or the whole save fails with `502 { "error": "ghost_update_failed" }` and the record is untouched. A save touching none of the pushed fields makes **no** Ghost call. A profile with **no `ghostMemberId`** skips the push (there is no member to update; the reconciliation audit reports it as `missingGhostMember`).
- **Errors:** `428` if `If-Match` is missing; `412` if stale; `403` if the caller may not edit this record or included a field outside their powers; `422` on validation failure; `404` if no such brother; `502 { "error": "ghost_update_failed" }` if the Ghost-first push of a changed pushed field fails (Book untouched — decision N65).
...
- **Behavior — raising (first `true`):** snapshots the brother's consent/verification state (system-internal, DATABASE-SCHEMA §8), forces `allowNewsletterEmail` **off**, stamps `newsletterConsentChangedAt` (the D103 reconcile's causal signal), sets the deceased fields, and freezes verification (D48). The resulting subscription change is pushed to Ghost **first, Ghost-first-gated** (diff-based; decisions D96/N65, superseding N40's "push is Phase 5" deferral now that Phase 5b-1 wires it): a Ghost failure fails the action (`502 { "error": "ghost_update_failed" }`, Book untouched); a profile with no `ghostMemberId` skips the push (the reconciliation audit reports the missing member). **Reversing (`false`):** restores the snapshot (consent flag and verification exactly as captured — D80), pushes any resulting subscription re-enable to Ghost the same Ghost-first-gated way, stamps `newsletterConsentChangedAt` again, clears the snapshot and the deceased fields. *(The former companion flag `allowCommentReplyEmail` was removed — DECISIONS N68.)*
- **Response 200:** the updated record at the caller's projection, with a fresh `ETag`.
- **Errors:** `404` if no such brother; `422` on invalid deceased-field values; `403` if the caller is a brother; `502 { "error": "ghost_update_failed" }` if the Ghost-first push fails (Book untouched — decision N65).
...
- **Errors:** `404` if no profile with this id exists; `422` on an invalid role value; `409 { "error": "last_admin" }` if the change would demote the only remaining admin.
