...
**⚠ Bulk-loader obligation — the privacy block (decision D163).** The loader must write **all five `privacy` flags `true`** for every brother it creates, matching the schema defaults in DATABASE-SCHEMA §3.3. This is easy to get silently wrong in two directions. The loader does **not** go through the Add Brother route, so it does not inherit that route's `DEFAULT_PRIVACY` for free; and the two conservative all-`false` blocks in the codebase (`data/cache.ts`, `pages/profile/patch.ts`) are *fail-closed fallbacks*, not defaults, so copying either would land every real brother opt-out. Decision D163 reversed the emergency/spouse default precisely so that brothers who record that information have it shared — a loader that writes `false` would make that reversal a no-op for all ~1,199 production records while leaving the code looking correct. **Verify on loaded data at cutover, not by re-reading the constant.**
