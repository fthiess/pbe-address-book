...
| **Ghost pull-and-seed utility** | One-time read from Ghost of the member ID (→ `ghostMemberId`), notes (→ `adminNote`), and current newsletter-subscription state (→ `allowNewsletterEmail`) — seeding the preference *from* Ghost so existing opt-outs are honored. (The comment-reply preference is no longer a Book field — DECISIONS N68.) | New (this session) | **Deferred to Stage 5 (OFC-340; D180)** — until it runs the Book→Ghost push no-ops |
| **Initial bulk-loader** | Write the merged Constitution-roster + MITAA + Ghost + family-tree + yearbook dataset into Book, including `ghostMemberId` (which the normal admin CSV import does **not** accept). | `apps/api/src/tools/csv-to-snapshot.ts` + `load-headshots.ts` → the D101 restore | **Built (D180)** — convert, then restore |
