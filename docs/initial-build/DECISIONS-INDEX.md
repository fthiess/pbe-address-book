...
- Bundle discipline: **D74** → ceiling raised 250 → **270 KB** brotli in 7a-2 (**N124**), attributable to Mixpanel's core build (+30.5 KB; measured 223.8 → 254.7 KB). Each move of that number names the change that caused it (OFC-286), and 7b's Lighthouse baseline is where it should next be re-derived from evidence. Slow-load UX: **D119** (threshold-gated loading overlay).
