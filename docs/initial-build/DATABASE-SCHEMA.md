...
User-submitted bug reports (decision D121), one document per report, created by `POST /api/bug-report` and reviewed by an administrator (Admin page / the audit log, ENGINEERING-DESIGN §6.1):
...
interface BugReport {
  ...
  page: string;                        // the SPA route the report was filed from
  description: string;                 // free text; trimmed; length-capped; treated as untrusted
...
There is **no outbound email**: a report is persisted and an audit entry written (decision D61), keeping the admin's inbox out of the attack surface; the endpoint is **rate-limited** (decision D86) and size-capped, and `description` is never interpolated into a dangerous sink. Reports are **admin-read only** and are not part of any profile projection.
...
- **Access control.** Headshots and thumbnails are served members-only from the CDN edge, gated by a Cloud CDN **signed cookie** issued at session establishment — a coarse grant, since any authenticated brother may view any brother's photos (decision D23; see ENGINEERING-DESIGN §2). The SPA builds the immutable object URLs itself from `id` and `headshotVersion`, so reads never touch the app backend. If the signed cookie has lapsed at the absolute 4-hour session cap (decision D22), an image read returns **`403`**; the SPA re-authenticates and **retries the image load** rather than rendering a broken image (decision D109; see ENGINEERING-DESIGN §2.5). Backup/restore of these objects follows the **offline-restore** model (decisions D100/D101) atop the GCS object versioning described above.
...
- **Majors maintenance UI** — *resolved in Session 6a (decision D69):* the `majors` vocabulary is maintained via a version-controlled source file applied by an idempotent seed/reconcile script (which also performs the initial load); a runtime admin curation UI is deferred to post-MVP.
