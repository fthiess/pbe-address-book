...
interface Profile {
  ...
  postPbeEducation?: string;           // free text, <= 120 chars (OFC-404)
  sports?: string;                     // free text, <= 120 chars (OFC-405)
  activities?: string;                 // free text, <= 120 chars (OFC-406)
  ...
  // --- Mentoring (PUBLIC read, consent write — see §9) ---
  willingToMentor: boolean;            // default false (opt-in, D166) — the brother has offered to provide professional information and advice. PUBLIC: every brother sees it, may column it, and may filter on it. Written by the owner or an admin only (consent-class write), never by a manager on another's record. Stored optionally: an absent value is `false`, normalized at the hydration boundary, so no backfill was needed. ⚠ Read via `isWillingToMentor` wherever it is *presented* — a deceased brother's stored opt-in is not a live offer
  ...
  // (allowCommentReplyEmail removed — DECISIONS N68: not settable via the Ghost Admin API, so a UI control would be non-functional; re-add tracked as OFC-215)
  ...
  // --- Housekeeping (lastModified restricted; verification public — OFC-207) ---
  lastVerifiedDate?: string;           // YYYY-MM-DD; present iff the profile is currently verified (system-managed, decision D28; PUBLIC read per OFC-207)
  verifiedBy?: BrotherId;              // Constitution ID of the verifier; set/cleared together with lastVerifiedDate (decision D28; PUBLIC read per OFC-207)
...
| `postPbeEducation` | `string?` | no | absent | public | Free text; max 120 chars (`MAX_SHORT_TEXT_LENGTH`). Rejected, never truncated. |
| `sports` | `string?` | no | absent | public | Free text; max 120 chars (`MAX_SHORT_TEXT_LENGTH`). Rejected, never truncated. |
| `activities` | `string?` | no | absent | public | Free text; max 120 chars (`MAX_SHORT_TEXT_LENGTH`). Rejected, never truncated. |
...
| `willingToMentor` | `boolean` | yes | `false` | **public** | Opt-in mentoring signal (decision D166, §9). Public on read but **consent-class on write** — owner or admin only, never a manager on another's record. ⚠ Present it via `isWillingToMentor` (`packages/shared/src/mentoring.ts`), which also excludes deceased brothers. |
...
| `verifiedBy` | `number?` | no | absent | public | Constitution ID of the most recent verifier; server-set with `lastVerifiedDate`. Public per OFC-207. |
...
- **`postPbeEducation`, `sports`, `activities`** (OFC-404/405/406) — free text, each at most **120 characters** (`MAX_SHORT_TEXT_LENGTH` in `packages/shared/src/validation.ts` — the single source both the web inputs' `maxLength` and this rule read). Measured on the **trimmed** value, so trailing whitespace alone cannot fail an at-cap entry. An over-length value is **rejected whole, never truncated** (Forrest's call): the brother keeps what he wrote and is asked to shorten it, rather than silently losing the tail. ⚠ The web editors also set the inputs' `maxLength`, so a brother in an ordinary browser cannot type past the cap at all and never reaches this rule — which is exactly why the rule must exist anyway, for any client that ignores the attribute.
...
- **Deceased mark / un-mark (snapshot & restore)** — Marking a profile deceased forces `allowNewsletterEmail` to `false` and freezes verification (`lastVerifiedDate`/`verifiedBy`, decisions D28/D48). Because the deceased flag is **admin-reversible** for error-correction (decision D49), the prior consent flag and verification state are **snapshotted at mark-time** and **restored on un-marking** (the snapshot is cleared once restored) — so a mistaken mark-deceased does not silently and permanently unsubscribe a living brother from real Ghost mail (decision D80). The force-off and the restore each update `newsletterConsentChangedAt`. *(The former companion flag `allowCommentReplyEmail` was removed — DECISIONS N68.)*
...
- **public** — visible to all authenticated brothers: identity, names, class year, majors, links, employer, **`postPbeEducation`, `sports` and `activities`** (OFC-404/405/406 — free text a brother writes about himself expressly so other brothers can find him by it; each is filterable and a selectable Directory column, and a toggle class would have required a sixth `PrivacyFlags` switch to withhold data whose only purpose is to be seen), **the mentoring opt-in `willingToMentor`** (decision D166 — a signal the brother publishes *in order to be found*, so a restricted class would defeat its purpose; note its **write** rule is nonetheless the narrow consent one, owner-or-admin, since a manager must not volunteer another brother's time), Big Brother, deceased status (including the `birthYear`/`deathYear` shown only when deceased, decision D122), the headshot/thumbnail, **verification status** (`lastVerifiedDate`, `verifiedBy` — OFC-207, amends D28: an accuracy signal and a public administrative act, not PII; the *right to mark verified* stays owner/manager/admin, enforced on the write side), and the brother's **`role`** (decision D128, reversing OFC-139's staff-only plan: the staff roles are official contact points, not secret; the *right to change a role* stays admin-only, a **protected** write enforced on the write side — every authenticated brother may *see* who is staff, only an admin may *set* it). (`spousePartnerName` is **not** public — it moved to the **toggle** class as third-party data, decision D93.)
...
`allowNewsletterEmail` (may PBE News be emailed to this brother) defaults `true`, is **pushed to Ghost** — which actually sends the mail (ENGINEERING-DESIGN §5.1) — and is **forced to `false` when a brother is marked deceased**. (The former companion flag `allowCommentReplyEmail` was removed — it is not settable via the Ghost Admin API, DECISIONS N68.) `allowShareWithMITAA` (default `false`, opt-in — decision D89) is a **master switch** meaning "may the brother's own contact information be shared with MITAA": when `true`, the MITAA export includes the full own-contact set (email, phone, postal address) regardless of the brother-facing share-toggles, which target a different audience; when `false`, no contact information is shared, though name, class year, and public deceased status still flow (ENGINEERING-DESIGN §5.3). Mailman list-subscription flags remain intentionally out of the MVP schema; that integration stays deferred (decisions D11, D60). The full role/capability matrix is in PRD §4, and the projection is enforced server-side as described in ENGINEERING-DESIGN §2 (decisions D5, D19).

**The mentoring opt-in (decision D166).** `willingToMentor` (default `false`, opt-in) records that a brother has offered to provide professional information and advice. It is shipped **ahead of the mentoring programme itself** (OFC-255) to collect opt-ins while launch traffic is at its peak; until that programme exists it does nothing but display and filter. Three properties are easy to get wrong and are therefore stated here:

- **Public on read, consent on write.** It is the only field whose read class is wide and whose write class is narrow. The asymmetry is deliberate: being findable is the point, but volunteering a brother's time is his call alone, so a manager editing another brother cannot change it (only the owner or an admin can) — the same rule `unlisted` follows under D124.
- **Absence is `false`.** The field stores optionally and is normalized to a concrete boolean at the single Firestore→`Profile` hydration boundary, so records written before it existed — every staging record, and everything the Phase 8 bulk-loader writes — are simply "not willing". No backfill is required, and none should be added: absence and declining are the same state by design.
- **Presentation is not the stored value.** Every surface that *shows* or *filters on* the opt-in goes through `isWillingToMentor` (`packages/shared/src/mentoring.ts`), which additionally excludes deceased brothers — "willing to provide advice" is present-tense, and a memorial profile must not carry it. The stored value is deliberately left untouched when a brother is marked deceased (unlike `allowNewsletterEmail`, which drives an outbound action and so is forced **off** at mark-deceased, its prior value snapshotted for restoration on un-marking — D80), and the CSV export (§10) deliberately carries the **stored** value, since an export is a dump of state.

In effect the classes yield **three reading projections** over directory data, plus the staff/system fields layered on top: an ordinary **brother** sees public fields (now including verification status) plus another brother's toggle fields where shared; a **manager** sees all of that plus every record's restricted flags and preferences (read-only) and the staff-internal `adminNote` (read/write), but still not the contact values behind an off-toggle; an **admin** sees everything, including those hidden contact values, and is the only role that can alter another brother's consent or privacy settings. The owner sees their own full record *except* `adminNote`. No client of any role ever receives `ghostMemberId`.
...
> **This section defines the canonical, round-trippable export — the "Export all data" file.** Since decision D176 the Directory's Export control offers a **second** CSV, *displayed columns*, which writes the columns the user has on screen using the grid's own headers and cell strings ("United States" rather than `US`, one Canonical Name rather than discrete name fields, a blank cell rather than an em-dash). That file is deliberately **not** governed by this section and is **not** an import format; it is built by `apps/web/src/pages/directory/displayed-csv.ts` from the column model, and shares only this section's cell formatting — the RFC-4180 escaping and the formula-injection neutralisation, via the same `formatCsvCell`.
...
- **Export-only (ignored on import):** `lastVerifiedDate` and `verifiedBy` appear as **read-only reporting columns** but are **ignored on import** — verification is system-managed and recomputed by the import-as-non-owner-edit rule (ENGINEERING-DESIGN §6.5, decisions D28, D68). Since OFC-207 these columns ride **every role's export**, brothers included (verification is public); before that they were manager/admin-only. Including `verifiedBy` restores symmetry with `lastVerifiedDate`: it is brother-visible on screen, so omitting it from the export would be an inconsistency (finding C8).
