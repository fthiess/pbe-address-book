...
**The roster** is a CSV with columns `profileId`, `firstName`, `lastName`, `classYear`, `email`, `role` — only the two name fields and `email` are required. Because testers appear under their real names, **the roster must never enter the public repo**: it lives as a **private GCS object**, `gs://pbe-book-staging-uat/roster/uat-testers.csv`, in a bucket the Book runtime service account cannot read. The deploy workflow already authenticates to that project via WIF, so CI reads it with no new secrets; the same file works for local runs. Forrest edits it with a one-line `gcloud storage cp`.

**Ids come from their own block, `TESTER_ID_FLOOR` (#9001+)** — testers no longer overwrite generated profiles (D156, amending the original design here). A blank `profileId` is assigned sequentially from that floor, so a roster of any size from zero to dozens works without hand-numbering. This is what keeps every deliberate generated fixture intact: the Canonical Name collision pair at #5001/#5002, the nominal and usable admins at #5003/#5004, and the six managers.

**Roles**: the **first data row is the default test admin** — normally Forrest — and every later row defaults to `brother`. An explicit `role` overrides either, which is also how the manager upgrade below survives a reseed.

**What the tool does** for each roster row, idempotently: write a profile carrying the tester's real name, class year, email and role, listed, living and email-visible, and **nothing else** — no address, professional fields, links or Big Brother — because *"fill in your profile" is deliberately the testers' first task* (Forrest's call, D132: account setup becomes end-to-end coverage of the edit path, the highest-value flow to watch real users attempt). It then reconciles the matching **ghost-staging members** so each tester can sign in through the real bridge, and writes the `ghostMemberId` back.

⚠ **Ghost matching is by email; deleting is by label.** Ghost enforces email uniqueness globally, so a pre-existing account holding a roster address is *adopted* rather than duplicated — the first cut scoped matching to the `book-uat-tester` label alone and 422'd against Forrest's own pre-existing account. Deletion stays label-scoped, so the tool can only ever remove members it created. A member that is neither in the roster nor labelled is invisible to the reconcile.

**Removing testers** needs no special path: `seed:staging` wipes `profiles` before the tool runs, so a row dropped from the CSV simply never comes back, and its labelled Ghost member is deleted as an orphan. Editing the CSV and reseeding is the whole management story — including shrinking the cohort back to Forrest alone.

**Wiring:** the tool runs in the deploy workflow after `seed:staging` / `seed:staging-images`, so *any* reseed — automatic or manual — lands with the testers present. It sits after the `Set up gcloud` step rather than inside the reseed block, because it needs `gcloud secrets` for the Ghost Admin key, and still before the Cloud Run deploy so the new revision cold-starts with the testers' emails in the index. It subsumes the old `STAGING_TESTER_EMAIL` mechanism: Forrest's own admin account is simply roster row 1.
...
**Status (2026-07-27; DECISIONS D154). DONE — no payment needed.** Ghost-staging now sends from the **verified Mailgun sending domain `staging.pbe400.org`**, on Mailgun's free tier: the final tester count is six, so the free plan's 100-messages-per-day ceiling and its one-custom-domain allowance both have ample headroom, and the "short-term paid account" of the baseline plan was never required. Live-confirmed end to end — a magic link delivered to an address that had never been a Mailgun authorized recipient, and signed a Ghost-staging member in. ⚠ **The safety model changed with it**: the sandbox's provider-side rejection is gone, so the guarantee that staging can never reach a real brother now rests on the fake member list and on `bulkEmail` remaining unset. D154 records both, and is required reading before any change to Ghost-staging's mail configuration.
