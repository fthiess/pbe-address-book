...
| **PL-14** | Opus | OFC-382, 381 | **Pre-Dec-7 client-staleness hardening**: version toast re-arms for later deploys; the SPA↔API compatibility epoch with a 426 forced refresh (epoch placement is Forrest's fork). **Must land before the Dec 7 newsletter**, the next load event. |
