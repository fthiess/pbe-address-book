...
- **No PII on any publicly viewable surface — and a repo is one.** Real names, emails, phone numbers, addresses, anything that identifies a living person: out of the tree, out of **commit messages**, out of **PR and issue titles and bodies**, out of published docs and sites. Write "the reporter", "a tester", "a user" instead; the private tracker is the right home for a name, and keeping it there costs nothing. **Redact on the way out**, while writing the artifact — not afterwards, because rewriting published history costs more than it recovers (a force-push invalidates every SHA cited from the tracker, from PR attachments, and from the docs), so the residue usually has to be left in place and documented instead. Two traps: the surfaces most often missed are the ones **outside** the tree, since a commit message and a PR body are published the moment they're pushed and no `.gitignore`, reviewer, or CI step ever looks at them; and the usual leak path is **attribution**, where a name arrives quoted from a bug report or ticket and reads as a citation rather than as PII — which is exactly why it slips through. ⚠ **No checker can enforce this** — one would have to hold the very list of real names it exists to keep out of a public repo — so it rests on being written into the project's `CLAUDE.md` and honored here.
