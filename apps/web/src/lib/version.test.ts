 ...
 */
describe("isNewerVersion (OFC-63)", () => {
  it("prompts when the deployed id differs from the loaded id", () => {
    expect(isNewerVersion("abc123", "def456")).toBe(true);
  });

  it("does not prompt when the ids match", () => {
    expect(isNewerVersion("abc123", "abc123")).toBe(false);
  });

  it("does not prompt on an absent or empty deployed id (a failed/dev poll)", () => {
    expect(isNewerVersion("abc123", null)).toBe(false);
    expect(isNewerVersion("abc123", "")).toBe(false);
  });
});
